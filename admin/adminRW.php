<!DOCTYPE html>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-+0n0xVW2eSR5OomGNYDnhzAbDsOXxcvSN1TPprVMTNDbiYZCxYbOOl7+AMvyTG2x" crossorigin="anonymous">
<link rel="stylesheet" href="../styles.css">
<html lang="en">

<header>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Project Database</title>

    <div class="container">
        <h1 class="logo">Admin Module</h1>

        <nav>
            <ul>
            <li><a href="adminRA.php">Register Admin</a></li>
                <li><a href="adminRL.php">Register Lecturer</a></li>
                <li><a href="adminRS.php">Register Student</a></li>
                <li><a href="adminRSub.php">Register Subject</a></li>
                <li><a href="adminRW.php">Register Workload</a></li>
                <li><a href="../index.php">Log Out</a></li>
            </ul>
        </nav>

    </div>
</header>

<body>
    <h3 style="margin-left: 120px; margin-top: 20px;">Register Workload</h3>
    <hr>
    <div class="container">
        <table id="tablestyle">
            <tr>
                <th>Lecturer Name</th>
                <th>Subject Name</th>
                <th style="text-align: center;">Update / Delete / Add</th>
            </tr>

            <tr>
                <td>Muhammad Hazim bin Mohd Alim</td>
                <td>Database</td>
                <td style="text-align: center;">
                    <input style="margin: 5px;" type="submit" value="Update"><input type="submit" value="Delete">
                </td>
            </tr>
            <tr>
                <td>
                    <select name="name">
                            <option value="volvo">Hazim</option>
                            <option value="saab">Saab</option>
                            <option value="mercedes">Mercedes</option>
                            <option value="audi">Audi</option>
                    </td>
                    <td>
                        <select name="subjectName">
                        <option value="volvo">Database</option>
                        <option value="saab">Creative and Innovative</option>
                        <option value="mercedes">Visual Programming</option>
                        <option value="audi">Mandarin</option>
                    </td>
                    <td style="text-align: center;">
                        <input type="submit" value="Add" style="width: auto;"></input>
                    </td>
                </tr>

        </table>


    </div>


</body>

</html>